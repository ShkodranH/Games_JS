...
for(let i = 0; i < 100; i++) {
    particles.push(new Particle({ 
        ...
        velocity: { x: 0, y: 0.3 }, radius: Math.random() * 3, color: 'white'
    }));
}
...
function createParticles({ object, color, fades }) {
    for(let i = 0; i < 15; i++) {
        particles.push(new Particle({ 
            ...
            radius: Math.random() * 3, color: color || '#baa0de', fades
        }));
    }
}
            ...
            projectiles.forEach((projectile, j) => {
                ...
                && projectile.position.y + projectile.radius >= invader.position.y) {
                    setTimeout(() => {
                        ...
                        const projectileFound = grid.invaders.find(projectile2 => projectile2 === invader);
